...
class QRCore {
    ...
    downloadPDF(filename) {
        if (typeof window.jspdf === 'undefined') {
            console.error('jsPDF not loaded, falling back to PNG');
            this.downloadPNG(filename);
            return;
        }
        const { jsPDF } = window.jspdf;
        const imgData = this.currentQR.toDataURL('image/png');

        // A4 page in mm
        const pageWidth = 210;
        const pageHeight = 297;
        const qrSize = 100; // QR code size in mm
        const x = (pageWidth - qrSize) / 2;
        const y = (pageHeight - qrSize) / 2 - 20;

        const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
        doc.setFontSize(14);
        doc.text(filename, pageWidth / 2, y - 8, { align: 'center' });
        doc.addImage(imgData, 'PNG', x, y, qrSize, qrSize);
        doc.setFontSize(10);
        doc.setTextColor(120);
        doc.text('Generated by ToolzzHub.com', pageWidth / 2, y + qrSize + 10, { align: 'center' });
